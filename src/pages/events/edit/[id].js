...
export default function EditEvent() {
  ...
  const fetchEvent = async () => {
    try {
      ...
      setFormData({
        ...
        hora_inicio: fechaInicio.toTimeString().slice(0, 5),
        duracion_estimada: data.duracion_estimada ? data.duracion_estimada.replace(' hours', '') : '',
        direccion_completa: data.direccion_completa || '',
        link_google_maps: data.link_google_maps || '',
        aforo_maximo: data.aforo_maximo ? data.aforo_maximo.toString() : '',
        precio: data.precio ? data.precio.toString() : '',
        restriccion_tipo: data.restriccion_tipo || '',
        restriccion_tamaño: data.restriccion_tamaño || '',
        foto_portada: data.foto_portada || '',
        permite_comentarios: data.permite_comentarios || true,
        visibilidad: data.visibilidad || true
      })
    } catch (error) {
  ...
  return (
    <>
      <Head>
        <title>Editar Evento - PetsEvents</title>
      </Head>
